...
import { DEFAULT_NEWS_SETTINGS, loadNewsSettings, type NewsSettings } from '../../lib/newsSettings';
...
export function NewsPanel() {
  ...
  const fetchNews = useCallback(async (currentSettings: NewsSettings) => {
    try {
      ...
      const activeSources = (currentSettings.sources ?? []).filter((s) =>
        currentSettings.enabledSources.includes(s.id),
      );
  ...
  useEffect(() => {
    ...
    refreshRef.current = setInterval(() => void fetchNews(settings), settings.refreshMinutes * 60_000);
  ...
  return (
    <section className="news-panel" aria-label="Painel de notícias">
      {/* Header */}
      ...
      <div className="news-panel__search">
        <label htmlFor="news-keyword-filter" className="sr-only">
          Filtrar notícias por palavra-chave
        </label>
        <Search size={14} />
        <input
      ...
      {loading && allItems.length === 0 ? (
        ...
        <div className="news-feed">
          ...
          {items.length === 0 && allItems.length > 0 && settings.keywords && (
            <p className="news-panel__empty">
              Nenhuma notícia corresponde aos filtros: &ldquo;{settings.keywords}&rdquo;
            </p>
          )}
      ...
      {fetchedAt && (
        <p className="news-panel__footer">
          Atualizado {timeAgo(fetchedAt)} · {settings.enabledSources.length} fonte
          {settings.enabledSources.length !== 1 ? 's' : ''} · atualização a cada {settings.refreshMinutes}min
        </p>
      )}
    </section>
  );
