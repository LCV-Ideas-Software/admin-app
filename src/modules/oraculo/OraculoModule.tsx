...
export function OraculoModule() {
    ...
    async (notify = false) => {
      ...
      try {
        ...
        const data = (await res.json()) as {
          ok: boolean;
          total: number;
          items: RegistroLciLca[] | RegistroTesouroIpca[];
        };
  ...
  const carregarModelos = useCallback(async () => {
    ...
    try {
      ...
      /* silencioso — manterá inputs manuais */
    } finally {
  ...
  const dispararCSV = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setCsvTriggerResult({
          ok: true,
          msg: `${data.titulos?.length ?? 0} títulos extraídos · Taxa média: ${data.taxaMediaIndicativa}% · Ref: ${data.dataReferencia}`,
          ms,
        });
        setTaxaCache(data);
  ...
  const carregarUserData = useCallback(
    async (notify = false) => {
      ...
      <div className="inline-actions" style={{ marginBottom: '1rem' }}>
        {tabs.map(({ id, label, icon }) => (
          <button
            key={id}
            type="button"
            className={activeTab === id ? 'primary-button' : 'ghost-button'}
          ...
          <div className="result-toolbar">
            <div>
              <h4>
                {activeTab === 'lci-lca' ? (
                  <>
                    <Database size={16} /> Registros LCI/LCA
                  </>
                ) : (
                  <>
                    <Globe size={16} /> Registros Tesouro IPCA+
                  </>
                )}
              </h4>
            ...
            <div className="inline-actions">
              <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
                ...
                <input
                  ...
                  style={{ paddingLeft: '2rem', minWidth: '160px' }}
                />
              </div>
              <button
                type="button"
                className="ghost-button"
                onClick={() => void carregarRegistros(true)}
                disabled={loading}
              >
                  ...
                  <li key={r.id} className="post-row">
                    <div className="post-row-main">
                      <div className="flex-row-center">
                        <strong>
                          {activeTab === 'lci-lca'
                            ? `R$ ${r.aporte?.toLocaleString('pt-BR')}`
                            : `R$ ${r.valorInvestido?.toLocaleString('pt-BR')}`}
                        </strong>
                      </div>
                    ...
                    <div className="post-row-actions">
                      <button
                        ...
                        onClick={() => setConfirmDelete({ show: true, id: r.id, label })}
      ...
      {activeTab === 'usuarios' && (
        <article className="result-card">
          <div className="result-toolbar">
            <div>
              <h4>
                <Mail size={16} /> Dados Salvos por Usuários
              </h4>
          ...
          {selectedUser ? (
            (() => {
              ...
              try {
                parsed = JSON.parse(selectedUser.dadosJson);
              } catch {
                /* */
              }
                  ...
                  {hasParams && (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                        ...
                        {paramDuration > 0 && (
                          <span className="badge badge-planejado">Duration: {paramDuration} anos</span>
                        )}
                        {paramTaxaAtual > 0 && (
                          <span className="badge badge-em-implantacao">
                            Taxa IPCA+ atual: {paramTaxaAtual.toFixed(2)}%
                          </span>
                        )}
                        {paramPrazoDias > 0 && (
                          <span className="badge badge-planejado">Prazo LCI: {paramPrazoDias}d</span>
                        )}
                        {paramTaxaLci > 0 && (
                          <span className="badge badge-em-implantacao">LCI/LCA: {paramTaxaLci.toFixed(2)}% CDI</span>
                        )}
                        {paramAporte > 0 && (
                          <span className="badge badge-em-implantacao">
                            Aporte: R$ {paramAporte.toLocaleString('pt-BR')}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      <ul className="result-list" style={{ maxHeight: '400px', overflow: 'auto' }}>
                        {tRegs.map((r, i) => {
                          ...
                          return (
                            <li
                              // biome-ignore lint/suspicious/noArrayIndexKey: read-only parsed JSON list, stable order
                              key={i}
                              className="post-row"
                              style={{ borderLeft: `3px solid ${isSell ? '#dc2626' : '#16a34a'}` }}
                            >
                              <div className="post-row-main">
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                  <strong>
                                    R${' '}
                                    {Number(r.valorInvestido ?? 0).toLocaleString('pt-BR', {
                                      minimumFractionDigits: 2,
                                    })}
                                  </strong>
                                  <span
                                    className={`badge ${isSell ? 'badge-danger' : 'badge-em-implantacao'}`}
                                ...
                                <div className="post-row-meta">
                                  ...
                                  <span className="badge badge-em-implantacao">
                                    {Number(r.taxaContratada ?? 0).toFixed(2)}% a.a.
                                  </span>
                                  {r.vencimento ? (
                                    <span className="badge badge-planejado">Venc: {String(r.vencimento)}</span>
                                  ) : null}
                                </div>
                                {String(r.analise ?? '') && (
                                  <p style={{ margin: '6px 0 0', fontSize: 12, color: '#888', fontStyle: 'italic' }}>
                                    {String(r.analise)}
                                  </p>
                                )}
                              </div>
                      ...
                      {tRegs.length > 1 &&
                        (() => {
                          ...
                          const taxaMedia =
                            totalInvestido > 0
                              ? tRegs.reduce(
                                  (s, r) => s + Number(r.taxaContratada ?? 0) * Number(r.valorInvestido ?? 0),
                                  0,
                                ) / totalInvestido
                              : 0;
                          return (
                            <div style={{ marginTop: 8, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                              <span className="badge badge-em-implantacao">
                                Total: R$ {totalInvestido.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                              </span>
                          ...
                          <li key={i} className="post-row">
                            <div className="post-row-main">
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <strong>
                                  R$ {Number(r.aporte ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                </strong>
                              </div>
                              <div className="post-row-meta">
                                <span>{Number(r.prazoDias ?? 0)} dias</span>
                                <span className="badge badge-em-implantacao">
                                  {Number(r.taxaLciLca ?? 0).toFixed(2)}% CDI
                                </span>
                                <span className="badge badge-planejado">
                                  ≈ CDB {Number(r.cdbEquivalente ?? 0).toFixed(2)}%
                                </span>
                                <span className="badge badge-em-implantacao">
                                  IR: {Number(r.aliquotaIr ?? 0).toFixed(1)}%
                                </span>
                              </div>
                            </div>
                          </li>
                      ...
                      <li key={row.id} className="post-row">
                        ...
                        <div className="post-row-actions">
                          <button
                            type="button"
                            className="ghost-button danger"
                            onClick={() => setConfirmDelete({ show: true, id: row.id, label: row.email })}
        ...
        <>
          ...
          <article className="result-card">
            <div className="result-toolbar">
              <div>
                <h4>
                  <Database size={16} /> Status do Cache — Taxas IPCA+
                </h4>
              ...
              <div className="inline-actions">
                <button
                  type="button"
                  className="ghost-button"
                  onClick={() => void carregarStatusCache()}
                  disabled={taxaCacheLoading}
                >
              ...
              <div className="form-grid" style={{ paddingTop: '0.5rem' }}>
                <div className="field-group">
                  ...
                  <p
                    style={{
                      fontWeight: 700,
                      color: taxaCache.ok ? 'var(--success, #34a853)' : 'var(--danger, #ea4335)',
                      margin: 0,
                    }}
                  >
                    ...
                    <div className="field-group">
                      ...
                      <p style={{ margin: 0 }}>
                        {taxaCache.fonte === 'cache'
                          ? 'Cache D1'
                          : taxaCache.fonte === 'cache-stale'
                            ? 'Cache D1 (desatualizado)'
            ...
            {taxaCache?.ok && taxaCache.titulos && taxaCache.titulos.length > 0 && (
              <details style={{ marginTop: '0.75rem' }}>
                ...
                <ul className="result-list astro-akashico-scroll" style={{ marginTop: '0.5rem', maxHeight: '220px' }}>
                  {taxaCache.titulos.map((t) => (
                    <li key={`${t.tipo}-${t.vencimento}`} className="post-row">
                      <div className="post-row-main">
          ...
          <article className="result-card">
            <div className="result-toolbar">
              <div>
                <h4>
                  <Download size={16} /> Processamento Manual do CSV
                </h4>
                <p className="field-hint">
                  Re-download imediato do CSV (~13 MB) e atualização do cache, independente do cron.
                </p>
              </div>
              <div className="inline-actions">
                <button
                ...
                >
                  {csvTriggerLoading ? (
                    ...
                    <>
                      <Download size={16} /> Disparar Agora
                    </>
                  )}
                </button>
              </div>
            ...
            {csvTriggerResult && (
              ...
              >
                ...
                {csvTriggerResult.ms != null && (
                  <span style={{ marginLeft: '0.5rem', opacity: 0.6 }}>({csvTriggerResult.ms}ms)</span>
                )}
          ...
          <form className="form-card" onSubmit={(e) => e.preventDefault()}>
            <div className="result-toolbar">
              <div>
                <h4>
                  <Globe size={16} /> Fonte de Dados e Automação
                </h4>
            ...
            <fieldset className="settings-fieldset">
              ...
              <div className="field-group">
                ...
                <p className="field-hint">
                  ...
                  <a
                    href="https://www.tesourotransparente.gov.br/ckan/dataset/taxas-dos-titulos-ofertados-pelo-tesouro-direto"
                    target="_blank"
            ...
            <fieldset className="settings-fieldset">
              <legend>
                <Clock size={14} style={{ verticalAlign: 'middle' }} /> Atualização Automática (Cron)
              </legend>
              <p className="field-hint" style={{ marginBottom: '0.75rem' }}>
                ...
                {cronLoading && (
                  <>
                    {' '}
                    <Loader2 size={12} className="spin" style={{ verticalAlign: 'middle' }} /> Carregando…
                  </>
                )}
              </p>
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.75rem', flexWrap: 'wrap' }}>
                <div className="field-group" style={{ flex: '0 0 auto' }}>
                  ...
                  <select
                    ...
                    style={{ width: '80px' }}
                    onChange={(e) => {
                      setCronHour(Number(e.target.value));
                      setCronDirty(true);
                    }}
                  >
                    {Array.from({ length: 24 }, (_, i) => (
                      // biome-ignore lint/suspicious/noArrayIndexKey: hours 0..23 — index IS the value
                ...
                <div className="field-group" style={{ flex: '0 0 auto' }}>
                  ...
                  <select
                    ...
                    style={{ width: '80px' }}
                    onChange={(e) => {
                      setCronMinute(Number(e.target.value));
                      setCronDirty(true);
                    }}
                  >
              ...
              <p className="field-hint" style={{ marginTop: '0.5rem' }}>
                ...
                {' · '}Execução diária às{' '}
                <strong>
                  {pad(cronHour)}:{pad(cronMinute)} BRT
                </strong>{' '}
                ({pad(brtToUtc(cronHour))}:{pad(cronMinute)} UTC)
              </p>
            </fieldset>
          </form>
          ...
          <form className="form-card" onSubmit={(e) => e.preventDefault()}>
              ...
              <div>
                ...
                <p className="field-hint">
                  ...
                  {modelsLoading && (
                    <>
                      {' '}
                      <Loader2 size={12} className="spin" style={{ verticalAlign: 'middle' }} /> Carregando modelos…
                    </>
                  )}
            ...
            <fieldset className="settings-fieldset">
              ...
              <div className="form-grid">
                {renderModelSelect('Modelo Vision (OCR de extratos)', 'model-vision', config.modeloVision, (v) =>
                  handleSaveConfig({ modeloVision: v }),
                )}
                {renderModelSelect('Modelo Análise (Fiduciário)', 'model-analise', config.modeloAnalise, (v) =>
                  handleSaveConfig({ modeloAnalise: v }),
                )}
              </div>
            </fieldset>
          </form>
          ...
          <div className="admin-modal-overlay" role="dialog" aria-modal="true" aria-label="Confirmar exclusão">
            <div className="admin-modal-content">
              ...
              <div className="admin-modal-header">
                ...
                <p className="admin-modal-subtitle">{confirmDelete.label}</p>
              </div>
