...
export function ConfigModule() {
  ...
  const [msAiModels, setMsAiModels] = useState<{ chat: string; summary: string; editor: string; import: string }>({
    chat: '',
    summary: '',
    editor: '',
    import: '',
  ...
  const updateNewsSettings = useCallback(
    (patch: Partial<NewsSettings>) => {
  ...
  const handleNewsSourceToggle = useCallback(
    (sourceId: string) => {
  ...
  const handleDiscoverKeyDown = useCallback(
  ...
  const sourceBadge = (src: RssSuggestion['source']) => {
    switch (src) {
      case 'curated':
        return { emoji: '📚', label: 'Diretório' };
      case 'google-news':
        return { emoji: '📰', label: 'Google News' };
      case 'gemini-ai':
        return { emoji: '🤖', label: 'Gemini AI' };
      case 'auto-detect':
        return { emoji: '🔍', label: 'Auto-detect' };
    }
  };
  ...
  const loadMainsiteSettings = useCallback(
    async (shouldNotify = false) => {
      ...
      try {
        ...
        const incomingAiModels =
          (payload.settings.aiModels as { chat?: string; summary?: string; editor?: string; import?: string }) || {};
        setMsAiModels({
  ...
  const saveAiModelsImmediately = useCallback(
    async (nextModels: { chat: string; summary: string; editor: string; import: string }) => {
      try {
  ...
  const handleAiModelChange = useCallback(
    (field: 'chat' | 'summary' | 'editor' | 'import', value: string) => {
      setMsAiModels((prev) => {
    ...
    <section className="detail-panel module-shell module-shell-config">
      ...
      <form className="form-card" onSubmit={saveConfig}>
        <div className="result-toolbar">
          <div>
            <h4>
              <Settings2 size={16} /> Preferências operacionais
            </h4>
            <p className="field-hint">
              Configuração local de trabalho para acelerar operações recorrentes no admin-app.
            </p>
            {hasUnsavedChanges && <span className="badge badge-planejado">Alterações não salvas</span>}
          </div>
        ...
        <div className="form-grid">
          <div className="field-group">
            ...
            <input
              ...
              onChange={(event) =>
                setConfig((current) => ({ ...current, defaultOverviewLimit: Number(event.target.value) || 1 }))
              }
              disabled={saving}
            />
          </div>
        ...
        <div className="form-grid">
          <div className="field-group">
            ...
            <select
              ...
              onChange={(event) =>
                setConfig((current) => ({ ...current, enableOperationalToasts: event.target.value === '1' }))
              }
              disabled={saving}
            >
          ...
          <div className="field-group">
            ...
            <select
              ...
              onChange={(event) =>
                setConfig((current) => ({ ...current, strictValidationMode: event.target.value === '1' }))
              }
              disabled={saving}
            >
              <option value="1">Ligado</option>
          ...
          <div className="field-group">
            ...
            <select
              ...
              onChange={(event) =>
                setConfig((current) => ({ ...current, requireConfirmBeforeDelete: event.target.value === '1' }))
              }
              disabled={saving}
            >
      ...
      <form className="form-card" onSubmit={handleSaveMainsiteSettings}>
        <div className="result-toolbar">
          <div>
            <h4>
              <Globe size={16} /> Ajustes do MainSite
            </h4>
          ...
          <div className="inline-actions">
            <button
              ...
              onClick={() => void loadMainsiteSettings(true)}
              disabled={msSettingsLoading || msSavingSettings}
            >
        ...
        <fieldset className="settings-fieldset">
          ...
          <label className="toggle-row">
            <input
              id="cfg-rotation-enabled"
              name="cfgRotationEnabled"
              type="checkbox"
              checked={msRotation.enabled}
              onChange={(e) => setMsRotation({ ...msRotation, enabled: e.target.checked })}
            />
          ...
          <div className="form-grid">
            <div className="field-group">
              ...
              <input
                id="cfg-rotation-interval"
                name="cfgRotationInterval"
                type="number"
                min={1}
                value={msRotation.interval}
                onChange={(e) => setMsRotation({ ...msRotation, interval: parseInt(e.target.value, 10) || 60 })}
                disabled={!msRotation.enabled}
              />
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className="settings-fieldset">
          ...
          <label className="toggle-row">
            <input
              id="cfg-allow-auto-mode"
              name="cfgAllowAutoMode"
              type="checkbox"
              checked={msAppearance.allowAutoMode}
              onChange={(e) => setMsAppearance({ ...msAppearance, allowAutoMode: e.target.checked })}
            />
        ...
        <fieldset className="settings-fieldset">
          ...
          <div className="form-grid">
            ...
            <div className="field-group">
              ...
              <select
                id="cfg-shared-font-family"
                name="cfgSharedFontFamily"
                value={msAppearance.shared.fontFamily}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, fontFamily: e.target.value } })
                }
              >
            ...
            <div className="field-group">
              ...
              <input
                id="cfg-shared-font-size"
                name="cfgSharedFontSize"
                value={msAppearance.shared.fontSize}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, fontSize: e.target.value } })
                }
                placeholder="Ex: 1.15rem"
              />
            </div>
            <div className="field-group">
              <label htmlFor="cfg-shared-title-font-size">Tamanho da Fonte dos Títulos (H1)</label>
              <input
                id="cfg-shared-title-font-size"
                name="cfgSharedTitleFontSize"
                value={msAppearance.shared.titleFontSize}
                onChange={(e) =>
                  setMsAppearance({
                    ...msAppearance,
                    shared: { ...msAppearance.shared, titleFontSize: e.target.value },
                  })
                }
                placeholder="Ex: 1.8rem"
              />
            </div>
            <div className="field-group">
              <label htmlFor="cfg-shared-body-weight">Peso do Corpo de Texto</label>
              <select
                id="cfg-shared-body-weight"
                name="cfgSharedBodyWeight"
                value={msAppearance.shared.bodyWeight || '500'}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, bodyWeight: e.target.value } })
                }
              >
            ...
            <div className="field-group">
              ...
              <select
                id="cfg-shared-title-weight"
                name="cfgSharedTitleWeight"
                value={msAppearance.shared.titleWeight || '700'}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, titleWeight: e.target.value } })
                }
              >
            ...
            <div className="field-group">
              <label htmlFor="cfg-shared-line-height">
                Altura de Linha (Corpo):{' '}
                <strong className="range-value">{msAppearance.shared.lineHeight || '1.9'}</strong>
              </label>
              <input
                id="cfg-shared-line-height"
                name="cfgSharedLineHeight"
                type="range"
                min={1.4}
                max={2.4}
                step={0.1}
                value={msAppearance.shared.lineHeight || '1.9'}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, lineHeight: e.target.value } })
                }
              />
            ...
            <div className="field-group">
              ...
              <select
                id="cfg-shared-text-align"
                name="cfgSharedTextAlign"
                value={msAppearance.shared.textAlign || 'justify'}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, textAlign: e.target.value } })
                }
              >
            ...
            <div className="field-group">
              ...
              <select
                id="cfg-shared-text-indent"
                name="cfgSharedTextIndent"
                value={msAppearance.shared.textIndent || '3.5rem'}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, shared: { ...msAppearance.shared, textIndent: e.target.value } })
                }
              >
            ...
            <div className="field-group">
              ...
              <select
                id="cfg-shared-paragraph-spacing"
                name="cfgSharedParagraphSpacing"
                value={msAppearance.shared.paragraphSpacing || '2.2rem'}
                onChange={(e) =>
                  setMsAppearance({
                    ...msAppearance,
                    shared: { ...msAppearance.shared, paragraphSpacing: e.target.value },
                  })
                }
              >
            ...
            <div className="field-group">
              ...
              <select
                id="cfg-shared-content-max-width"
                name="cfgSharedContentMaxWidth"
                value={msAppearance.shared.contentMaxWidth || '1126px'}
                onChange={(e) =>
                  setMsAppearance({
                    ...msAppearance,
                    shared: { ...msAppearance.shared, contentMaxWidth: e.target.value },
                  })
                }
              >
            ...
            <div className="field-group">
              ...
              <div className="theme-color-grid">
                <label className="color-label">
                  Cor ativa{' '}
                  <input
                    id="cfg-shared-link-color"
                    name="cfgSharedLinkColor"
                    type="color"
                    value={msAppearance.shared.linkColor || '#4da6ff'}
                    onChange={(e) =>
                      setMsAppearance({
                        ...msAppearance,
                        shared: { ...msAppearance.shared, linkColor: e.target.value },
                      })
                    }
                  />
                </label>
        ...
        <fieldset className="settings-fieldset">
          ...
          <div className="theme-color-grid">
            <label className="color-label">
              Cor de Fundo{' '}
              <input
                id="cfg-dark-bg-color"
                name="cfgDarkBgColor"
                type="color"
                value={msAppearance.dark.bgColor}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, dark: { ...msAppearance.dark, bgColor: e.target.value } })
                }
              />
            </label>
            <label className="color-label">
              Cor do Texto{' '}
              <input
                id="cfg-dark-font-color"
                name="cfgDarkFontColor"
                type="color"
                value={msAppearance.dark.fontColor}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, dark: { ...msAppearance.dark, fontColor: e.target.value } })
                }
              />
            </label>
            <label className="color-label">
              Cor dos Títulos{' '}
              <input
                id="cfg-dark-title-color"
                name="cfgDarkTitleColor"
                type="color"
                value={msAppearance.dark.titleColor}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, dark: { ...msAppearance.dark, titleColor: e.target.value } })
                }
              />
            </label>
          </div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                ...
                onChange={(e) =>
                  setMsAppearance({
                    ...msAppearance,
                    dark: { ...msAppearance.dark, bgImage: formatImageUrl(e.target.value) },
                  })
                }
                placeholder="https://..."
                style={{ flex: 1 }}
              ...
              <button
                ...
                onClick={() => {
                  setUploadTarget('dark');
                  fileInputRef.current?.click();
                }}
                disabled={isUploadingBg}
                title="Fazer upload para o Storage R2"
              >
                {isUploadingBg && uploadTarget === 'dark' ? (
        ...
        <fieldset className="settings-fieldset">
          ...
          <div className="theme-color-grid">
            <label className="color-label">
              Cor de Fundo{' '}
              <input
                id="cfg-light-bg-color"
                name="cfgLightBgColor"
                type="color"
                value={msAppearance.light.bgColor}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, light: { ...msAppearance.light, bgColor: e.target.value } })
                }
              />
            </label>
            <label className="color-label">
              Cor do Texto{' '}
              <input
                id="cfg-light-font-color"
                name="cfgLightFontColor"
                type="color"
                value={msAppearance.light.fontColor}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, light: { ...msAppearance.light, fontColor: e.target.value } })
                }
              />
            </label>
            <label className="color-label">
              Cor dos Títulos{' '}
              <input
                id="cfg-light-title-color"
                name="cfgLightTitleColor"
                type="color"
                value={msAppearance.light.titleColor}
                onChange={(e) =>
                  setMsAppearance({ ...msAppearance, light: { ...msAppearance.light, titleColor: e.target.value } })
                }
              />
            </label>
          </div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <input
                ...
                onChange={(e) =>
                  setMsAppearance({
                    ...msAppearance,
                    light: { ...msAppearance.light, bgImage: formatImageUrl(e.target.value) },
                  })
                }
                placeholder="https://..."
                style={{ flex: 1 }}
              ...
              <button
                ...
                onClick={() => {
                  setUploadTarget('light');
                  fileInputRef.current?.click();
                }}
                disabled={isUploadingBg}
                title="Fazer upload para o Storage R2"
              >
                {isUploadingBg && uploadTarget === 'light' ? (
                  <Loader2 size={16} className="spin" />
                ) : (
                  <Upload size={16} />
                )}
        ...
        <fieldset className="settings-fieldset" style={{ marginTop: '24px' }}>
          <legend>
            <BrainCircuit size={16} style={{ display: 'inline', verticalAlign: 'middle', marginRight: '6px' }} />{' '}
            Modelos de IA (Gemini)
          </legend>
          ...
          <div className="form-grid" style={{ gridTemplateColumns: 'minmax(0, 1fr)' }}>
            ...
            <div className="field-group">
              <label
                htmlFor="cfg-mainsite-ai-summary"
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
            ...
            <div className="field-group">
              <label
                htmlFor="cfg-mainsite-ai-editor"
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
              ...
              >
                ...
                {geminiModels.map((m) => (
                  <option key={`editor-${m.id}`} value={m.id}>
                    {m.displayName} ({m.api}) {m.vision ? '👁️' : ''}
                  </option>
                ))}
              </select>
            ...
            <div className="field-group">
              <label
                htmlFor="cfg-mainsite-ai-import"
                style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
              >
              ...
              >
                ...
                {geminiModels.map((m) => (
                  <option key={`import-${m.id}`} value={m.id}>
                    {m.displayName} ({m.api}) {m.vision ? '👁️' : ''}
                  </option>
                ))}
              </select>
      ...
      <article className="result-card">
        <header className="result-header">
          <h4>
            <Database size={16} /> Sync Manual — Controles por módulo
          </h4>
        </header>
        <div className="field-group">
      ...
      {selectedSyncModule === 'astrologo' && (
        <SyncStatusCard
          ...
          title="Sincronização do Astrólogo"
          description="Replica mapas pendentes com verificação prévia opcional."
        />
      )}
      {selectedSyncModule === 'calculadora' && (
        <SyncStatusCard
          ...
          title="Sincronização da Calculadora"
          description="Atualiza parâmetros de observabilidade e limites de uso."
        />
      )}
      {selectedSyncModule === 'mainsite' && (
        <SyncStatusCard
          ...
          title="Sincronização do MainSite"
          description="Validação e saneamento de posts e configurações com verificação prévia."
        />
      )}
      {selectedSyncModule === 'mtasts' && (
        <SyncStatusCard
          ...
          title="Sincronização do MTA-STS"
          description="Atualiza histórico, domínios e políticas de segurança de e-mail."
        />
      )}
      ...
      <article className="result-card">
        <header className="result-header">
          <h4>
            <Rocket size={16} /> Purge de Deployments — Cloudflare Pages
          </h4>
          <p className="field-hint">
            Varre todos os projetos Pages da conta e expurga deployments obsoletos, mantendo apenas o mais recente de
            cada projeto.
          </p>
        </header>
      ...
      <article className="form-card">
        <div className="result-toolbar">
          <div>
            <h4>
              <Newspaper size={16} /> Painel de Notícias
            </h4>
            <p className="field-hint">
              Configure as fontes, frequência de atualização e filtros do painel de notícias exibido na Visão Geral.
            </p>
          </div>
        </div>
        ...
        <div className="field-group">
          <div className="rss-sources-header">
            <p className="field-label">
              <strong>Fontes de notícias ativas ({newsSettings.sources.length})</strong>
            </p>
            <select
            ...
            >
              ...
              {sourceCategories.map((cat) => (
                <option key={cat} value={cat}>
                  {cat} ({newsSettings.sources.filter((s) => s.category === cat).length})
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="field-group" ref={dropdownRef}>
          <p className="field-label">
            ...
            {loadingSuggestions && (
              <Loader2 size={14} className="spin" style={{ marginLeft: 8, verticalAlign: 'middle' }} />
            )}
          </p>
          <p className="field-hint">
            <Zap size={12} style={{ verticalAlign: 'middle' }} /> Motor inteligente: digite em qualquer campo para
            descobrir fontes automaticamente.
          </p>
          {/* biome-ignore lint/a11y/noStaticElementInteractions: container-level keydown listener for Enter-to-discover; all child inputs own their own a11y */}
          <div className="news-settings__add-source" onKeyDown={handleDiscoverKeyDown}>
            <label htmlFor="news-new-source-name" className="sr-only">
              Nome da nova fonte RSS
            </label>
            <input
              ...
              onChange={(e) => {
                ...
                setActiveField('name');
                triggerDiscovery(e.target.value, 'name');
              }}
              onFocus={() => {
                setActiveField('name');
                if (newSourceName.trim().length >= 2) triggerDiscovery(newSourceName, 'name');
              }}
              autoComplete="off"
            />
            <label htmlFor="news-new-source-url" className="sr-only">
              URL da nova fonte RSS
            </label>
            <input
              ...
              onChange={(e) => {
                ...
                setActiveField('url');
                triggerDiscovery(e.target.value, 'url');
              }}
              onFocus={() => {
                setActiveField('url');
                if (newSourceUrl.trim().length >= 2) triggerDiscovery(newSourceUrl, 'url');
              }}
              autoComplete="off"
            />
            <label htmlFor="news-new-source-category" className="sr-only">
              Categoria da nova fonte RSS
            </label>
            <input
              ...
              onChange={(e) => {
                setNewSourceCategory(e.target.value);
                setActiveField('category');
                triggerDiscovery(e.target.value, 'category');
              }}
              onFocus={() => {
                setActiveField('category');
                if (newSourceCategory.trim().length >= 2) triggerDiscovery(newSourceCategory, 'category');
              }}
              autoComplete="off"
            />
            <button
