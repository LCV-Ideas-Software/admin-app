...
const normalizeCardsForCompare = (items: HubCard[]) =>
  items.map((card) => ({
    name: card.name.trim(),
...
export function HubCardsModule(props: HubCardsModuleProps) {
  ...
  const moduleToneClass = endpoint === '/api/apphub/config' ? 'module-shell-apphub' : 'module-shell-adminhub';
  ...
  const filteredCardEntries = useMemo(() => {
    ...
    return entries.filter(({ card }) => {
      const haystack = [card.name, card.description, card.url, card.badge, card.icon].join(' ').toLowerCase();
  ...
  const qualityMetrics = useMemo(() => {
    ...
    const invalidCards = cardFieldErrors.reduce(
      (acc, fieldErrors) => (Object.keys(fieldErrors).length > 0 ? acc + 1 : acc),
      0,
    );
    ...
    const status = score >= 95 ? 'ok' : score >= 70 ? 'warning' : 'error';
  ...
  const loadConfig = useCallback(
    async (shouldNotify = false) => {
      ...
      try {
        ...
        const nextPayload = await parseApiPayload<HubConfigPayload>(
          response,
          `Falha ao carregar configuração de ${title}`,
        );
    ...
    [adminActor, endpoint, showNotification, title, withTrace],
  );
  ...
  const moveCardToIndex = useCallback(
    ...
    <section className={`detail-panel module-shell ${moduleToneClass}`}>
      ...
      <form className="form-card" onSubmit={handleSave}>
        <div className="result-toolbar">
          <div>
            <h4>
              <Save size={16} /> Configuração de cards
            </h4>
            <p className="field-hint">Edite, ordene e configure os cards exibidos neste hub.</p>
            {hasUnsavedCards && <span className="badge badge-planejado">Alterações não salvas</span>}
          </div>
          <div className="inline-actions">
            ...
            <button
              ...
              disabled={disabled || !hasUnsavedCards}
            >
              <RefreshCw size={16} />
              Restaurar snapshot
            </button>
                    ...
                    {filteredCardEntries.map(({ card, index }) => {
                      ...
                      return (
                        <option key={`${adminActorFieldId}-opt-${index}`} value={index}>
                          #{index + 1} — {card.name.trim() || 'Novo card'}
                          {hasErrors ? ' ⚠️' : ''}
                        </option>
                      );
                    })}
                    ...
                    <article className="card-editor-item">
                      ...
                      <div className="form-grid">
                        ...
                        <div className="field-group">
                          ...
                          />
                          {errors.url && (
                            <span id={urlErrorId} className="field-error">
                              {errors.url}
                            </span>
                          )}
                        </div>
      ...
      <article className="result-card">
        <header className="result-header">
          <h4>
            <GripVertical size={16} /> Catálogo (paridade visual — arraste para reordenar)
          </h4>
        ...
        {cards.length === 0 ? (
          ...
          // biome-ignore lint/a11y/useSemanticElements: children are <button>; wrapping each in <li> would change the CSS grid layout
          <div className="catalog-grid" role="list" aria-label={`Catálogo reordenável de ${title}`}>
            {cards.map((card, index) => (
              <button
                // biome-ignore lint/suspicious/noArrayIndexKey: card.name may duplicate; index disambiguates for drag reorder
