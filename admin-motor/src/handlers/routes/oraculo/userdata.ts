...
export const onRequestDelete = async (context: HandlerContext) => {
  ...
  try {
    ...
    const row = (await db
      .prepare('SELECT email, dados_json FROM oraculo_user_data WHERE id = ? LIMIT 1')
      .bind(id)
    ...
    for (const lotId of tesouroIds) {
      const result = await db.prepare('DELETE FROM oraculo_tesouro_ipca_lotes WHERE id = ?').bind(lotId).run();
      if (getChanges(result) > 0) deletedCounts.lotes++;
    }

    // 4. Deletar registros LCI/CDB (por IDs extraídos do JSON)
    for (const regId of lciIds) {
      const result = await db.prepare('DELETE FROM oraculo_lci_cdb_registros WHERE id = ?').bind(regId).run();
      if (getChanges(result) > 0) deletedCounts.registros++;
    }

    // 5. Deletar todos os tokens de autenticação desse email
    const tokenResult = await db.prepare('DELETE FROM oraculo_auth_tokens WHERE email = ?').bind(email).run();
