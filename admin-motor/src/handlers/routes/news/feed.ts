 ...
 */
function parseRSSFeed(xmlText: string, sourceName: string, _sourceId: string, maxItems: number): NewsItem[] {
 ...
 */
async function fetchFeed(source: { id: string; name: string; url: string }, maxItems: number): Promise<NewsItem[]> {
  try {
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  if (customSourcesParam) {
    try {
      const parsed = JSON.parse(decodeURIComponent(customSourcesParam)) as Array<{
        id: string;
        name: string;
        url: string;
        category: string;
  ...
  const feedResults = await Promise.allSettled(activeSources.map((source) => fetchFeed(source, maxItems)));
