...
export async function onRequestPost(context: Context) {
  try {
    ...
    const { accountId } = await resolveCloudflarePwAccount(context.data?.env ?? context.env);

    // 1. Obter domínios atrelados ao projeto
    const pagesDomains = await listCloudflarePagesDomains(context.data?.env ?? context.env, accountId, projectName);
    ...
    const zones = await listCloudflareZones(context.data?.env ?? context.env);
    ...
    const purgePromises = Array.from(zoneToDomains.entries()).map(([zoneId, hosts]) =>
      purgeCloudflareZoneCache(context.data?.env ?? context.env, zoneId, { hosts }).catch((err) => {
