...
import { unauthorizedResponse, validatePutAuth } from '../_lib/auth';
import {
  type HubCard,
  logHubEvent,
  parseCardsFromBody,
...
export async function onRequestGet(context: Context) {
  ...
  try {
    const resolved = await resolveHubConfig(context.data?.env ?? context.env, 'apphub');
...
export async function onRequestPut(context: Context) {
  ...
  const authContext = await validatePutAuth(context.request, env.APPHUB_BEARER_TOKEN, {
  ...
  try {
    ...
    const updated = await saveCardsToDb(db, 'apphub', cards, adminActor);
