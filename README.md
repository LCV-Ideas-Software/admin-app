...
**Status.** Stable. Current internal source version: **APP v02.15.30**. The last
...
Revisions return one strict JSON object inside `<maestro_revision_report>`. Each changed received block has one `changed_blocks` entry with its original `block_id` and a nonempty `protocol_basis`. A `split` or `addition` that creates more than one extra block states the number in `new_block_count`; a one-block increase may omit it. The content lock rejects undeclared edits, reorderings and growth before accepting revised custody. The separate question of declaring the provenance of unmatched revised blocks remains tracked in ADMIAPP-29.
