...
**Status.** Stable. Current internal source version: **APP v02.15.29**. The last
...
Execution controls are explicit: every Maestro AI session requires a positive max cost and positive per-provider input/output rates before any paid provider call is made; an optional time limit can also be configured. The fixed peer models are `claude-fable-5-1`, `gpt-6-astra`, `gemini-3.1-pro-preview`, `deepseek-v4-pro`, `grok-4.7`, and `perplexity/sonar`. The Perplexity Agent API uses its official `xhigh` preset with its own Sonar model selected explicitly. Saved settings, new writes, and resumed calls normalize older IDs; the panel displays one read-only model per provider. The editable default input/output rate card (USD per million tokens) is Claude 10/50, Codex 10/50, Gemini 2/12, DeepSeek 1.32/3.96, Grok 2/6, and Perplexity 0.25/2.5, plus a Perplexity search request estimate. [DeepSeek's Pro prices](https://api-docs.deepseek.com/quick_start/pricing/) vary by time; the seed uses the peak rate. [Grok](https://docs.x.ai/developers/models/grok-4.7), [GPT-6 Astra](https://developers.openai.com/api/docs/models/gpt-6-astra), and [Gemini 3.1 Pro](https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing) charge higher rates beyond their long-context thresholds; the editable pre-call estimate does not automatically adjust to those tiers. Perplexity's completed responses use `usage.cost.total_cost` when available and record `cost_source` in the session journal. Gemini 3.1 Pro Preview requires `VERTEX_LOCATION=global`; regional locations fail clearly instead of selecting a Flash model. The editorial protocol is persisted in Cloudflare D1 `bigdata_db` through the `BIGDATA_DB` binding (`maestro_ai_settings`) and loaded automatically by every session. Link/evidence checks run inside the engine and are reported through session events.
