...
**Status.** Stable. Current internal source version: **APP v02.15.31**. The last
...
| Release         | Scope                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`APP v02.15.31`** | **Maestro AI: IDs de bloco e migração de sessões.** Declarações `changed_blocks` com IDs ausentes do manifesto recebido são rejeitadas antes de autorizar crescimento. A fixture JSON com dígitos árabes agora espera essa rejeição; `B10000` permanece válido quando existe. Um marcador independente dos modelos salvos recupera sessões de migrações parciais e evita varreduras repetidas. Versão interna de fonte, sem tag ou GitHub Release (ADMIAPP-30 / #653; ADMIAPP-33 / #656). |
| **`APP v02.15.30`** | **Maestro AI: migração segura dos modelos.** A configuração das sessões é atualizada quando os modelos salvos mudam; leituras seguintes não repetem a varredura. O rate card documenta o piso dos preços base e a estimativa separada de ferramentas da Agent API. Versão interna de fonte, sem tag ou GitHub Release (ADMIAPP-33 / #656). |
| **`APP v02.15.29`** | **Maestro AI: seis peers atuais e distintos.** Um modelo próprio e de maior capacidade por provedor: Claude Fable 5.1, GPT-6 Astra, Gemini 3.1 Pro Preview, DeepSeek V4 Pro, Grok 4.7 e Perplexity Sonar com preset `xhigh`. IDs superados e Flash deixam de ser configuráveis; configurações salvas e sessões retomadas são normalizadas. Versão interna de fonte, sem tag ou GitHub Release (ADMIAPP-32 / #655; ADMIAPP-33 / #656). |
| **`APP v02.15.28`** | **Maestro AI: JSON estrito e Agent API Perplexity.** A trava de conteúdo valida declarações estruturadas, identifica os blocos editados e limita o crescimento declarado; Perplexity usa o preset oficial `medium` da Agent API, registra o custo informado pelo provedor e mantém a estimativa quando ele omite o custo. Versão interna de fonte, sem tag ou GitHub Release. |
...
| **`v02.15.19`** | **`pageSize` não finito cai no padrão do servidor.** `NaN` atravessava o clamp (Math.trunc/min/max propagam NaN) e chegava à URL, reproduzindo o HTTP 400 que a normalização evita. |
| **`v02.15.18`** | **`pageSize` normalizado no intervalo aceito.** O clamp anterior cobria só o teto; um valor negativo continuaria indo na URL e reproduzindo o HTTP 400 que o hotfix evita. O cliente passa a normalizar para [0, 300], truncando frações. |
| **`v02.15.17`** | **Hotfix: catálogo de modelos voltou a responder.** `models.list` pedia `pageSize=1000`, acima do máximo de 300 do endpoint, e as quatro rotas de catálogo respondiam 500. O cliente passa a limitar o valor; a paginação segue trazendo o catálogo completo. Inclui o tratamento de `timeout <= 0` como prazo esgotado e a desconsideração do flag legado de credencial do Gemini no painel. |
...
| **`v02.15.12`** | **Maestro AI: refresh de gerações de modelos + paridade desktop.** Candidatos/fallbacks atualizados (GPT-5.6 Sol/Terra/Luna, Claude Fable 5 + Opus 5, Grok 4.5, DeepSeek V4, Gemini pro-only), migração one-shot dos defaults semeados no D1, rate card nos preços de lista atuais e port do `prompt_cache_key` OpenAI/Grok do desktop. |
| **`v02.15.11`** | **Override de `sharp` estendido ao `tlsrpt-motor`.** O v02.15.10 cobriu só o lock da raiz; o mesmo `sharp` <0.35.0 vivia no lock do `tlsrpt-motor` e mantinha o alerta #51 aberto — override 0.35.3 lá também. |
| **`v02.15.09`** | **Análises DNS degradam por breakdown.** O CF 1034 vira limite de plano genérico (repassa o motivo real da Cloudflare — 6h ou upgrade para `queryType`), e cada card de ranking indisponível no plano é marcado individualmente sem derrubar a aba; série temporal e códigos de resposta seguem no Free. |
...
| **`v02.15.04`** | **Guards sem brechas na criação e no console avançado.** Criar worker verifica existência antes do upload (o PUT da Cloudflare é upsert e sobrescreveria silenciosamente, inclusive produção) e o console avançado bloqueia métodos mutantes sobre os recursos de produção do próprio admin. |
| **`v02.15.03`** | **Menu lateral estável ao despinar.** O estado de hover da sidebar passou a ser controlado pelo React (a classe imperativa era apagada em re-renders), então o menu não some mais sob o cursor ao despinar; e2e 26/26. |
| **`v02.15.02`** | **Diagnósticos preservados e guards de exclusão.** Falhas de upstream respondem 500 (o edge da Cloudflare intercepta 502 e trocava o JSON diagnóstico por HTML); excluir os workers de produção do admin ou o projeto Pages `admin-app` passa a exigir a frase de confirmação, bloqueado no servidor. |
| **`v02.15.01`** | **Deploy destravado.** Remove o binding de Secrets Store `CLOUDFLARE_STORAGE` do motor até o secret existir no store (a Cloudflare valida o binding em tempo de deploy); o runtime já degrada com fallback e instruções exatas na interface. |
| **`v02.15.00`** | **Paridade com a Dashboard Cloudflare nos módulos CF DNS e CF P&W.** DNS: 21 tipos de registro, operações em lote, import/export BIND, DNSSEC, configurações de zona, ciclo de vida de zonas, análises de consultas e residuais do Registrar. Workers & Pages: criação, editor de código com deploy, versões com rollout gradual/rollback, settings e bindings completos, domínios/workers.dev, cron, Workers Builds, métricas GraphQL e paridade de Pages (build config, variáveis/bindings por ambiente, deployments, domínios). Armazenamento completo (KV, D1 com guardas do banco operacional, R2) com navegação cruzada de bindings. Base técnica: núcleo único de API Cloudflare com mensagens de erro diagnósticas, sondagem de capacidades com degradação graciosa e guardas de autoproteção. |
| **`v02.14.06`** | **Compatibilidade do relatório no Worker.** A limpeza defensiva da análise continua usando o parser nativo no navegador e recorre ao caminho sem DOM no motor administrativo, sem depender de tipos exclusivos da interface web.                                                                                                                                                                                                                                                                                                                                                                                   |
| **`v02.14.05`** | **Astrólogo — relatório estritamente interpretativo.** E-mail e exportação deixam de repetir explicações metodológicas e de mencionar a interface administrativa; estados incompletos permanecem compreensíveis em linguagem humana.                                                                                                                                                                                                                                                                                                                                                                               |
| **`v02.14.03`** | **Astrólogo — relatório sem detalhes internos.** Visualização, exportação e e-mail preservam os cálculos e a interpretação, mas ocultam versões, contratos, hashes, motores, perfis, proveniência e mensagens técnicas, inclusive em registros históricos.                                                                                                                                                                                                                                                                                                                                                           |
| **`v02.14.02`** | **Astrólogo — relatórios sem marcadores internos.** Arquivo Akáshico, resumo e e-mails HTML/texto removem sentinelas técnicas de análises históricas, com nova barreira imediatamente antes do Resend.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
Execution controls are explicit: every Maestro AI session requires a positive max cost and positive per-provider input/output rates before any paid provider call is made; an optional time limit can also be configured. The fixed peer models are `claude-fable-5-1`, `gpt-6-astra`, `gemini-3.1-pro-preview`, `deepseek-v4-pro`, `grok-4.7`, and `perplexity/sonar`. The Perplexity Agent API uses its official `xhigh` preset with its own Sonar model selected explicitly. Saved settings, new writes, and resumed calls normalize older IDs; the panel displays one read-only model per provider. The editable input/output rate card has a floor at the listed base prices (USD per million tokens); smaller saved values are raised to that floor. Its defaults are Claude 10/50, Codex 10/50, Gemini 2/12, DeepSeek 1.32/3.96, Grok 2/6, and Perplexity 0.25/2.5, plus a configurable USD 14 per 1,000 requests as a conservative pre-call allowance for Agent API tool use, not an Agent API request tariff. Perplexity bills Agent API web searches per invocation; the completed response supplies the actual cost. [DeepSeek's Pro prices](https://api-docs.deepseek.com/quick_start/pricing/) vary by time; the seed uses the peak rate. [Grok](https://docs.x.ai/developers/models/grok-4.7), [GPT-6 Astra](https://developers.openai.com/api/docs/models/gpt-6-astra), and [Gemini 3.1 Pro](https://cloud.google.com/gemini-enterprise-agent-platform/generative-ai/pricing) charge higher rates beyond their long-context thresholds; the editable pre-call estimate does not automatically adjust to those tiers. Perplexity's completed responses use `usage.cost.total_cost` when available and record `cost_source` in the session journal. Gemini 3.1 Pro Preview requires `VERTEX_LOCATION=global`; regional locations fail clearly because this Pro preview requires the global endpoint. The editorial protocol is persisted in Cloudflare D1 `bigdata_db` through the `BIGDATA_DB` binding (`maestro_ai_settings`) and loaded automatically by every session. Link/evidence checks run inside the engine and are reported through session events.

Revisions return one strict JSON object inside `<maestro_revision_report>`. Each changed received block has one `changed_blocks` entry with its original `block_id` and a nonempty `protocol_basis`. A `split` or `addition` that creates more than one extra block states the number in `new_block_count`; a one-block increase may omit it. The content lock rejects declarations whose block_id is absent from the received manifest, then rejects undeclared edits, reorderings and growth before accepting revised custody. The separate question of declaring the provenance of unmatched revised blocks remains tracked in ADMIAPP-29.
