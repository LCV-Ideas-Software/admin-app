...
export async function onRequestGet(context: Context) {
  ...
  try {
    const parametros = await readLatestParams(env.ITAU_SOURCE_DB)
    ...
    const message = error instanceof Error ? error.message : 'Falha ao carregar parâmetros do Itaú'
...
export async function onRequestPost(context: Context) {
  ...
  try {
    ...
    const atuais = await readLatestParams(env.ITAU_SOURCE_DB)
    ...
    for (const [chave, valor] of Object.entries(values)) {
      await env.ITAU_SOURCE_DB.prepare('INSERT INTO itau_parametros_customizados (chave, valor) VALUES (?, ?)')
        .bind(chave, String(valor))
        .run()
    }

    for (const mudanca of mudancas) {
      await env.ITAU_SOURCE_DB.prepare(`
    ...
    const message = error instanceof Error ? error.message : 'Falha ao salvar parâmetros do Itaú'
