...
export async function onRequestPost(context: Context) {
  ...
  try {
    ...
    if (!overviewResponse.ok) {
      throw new Error(`Falha no backend legado do Itaú (/overview): HTTP ${overviewResponse.status}`)
    }

    if (!rateLimitResponse.ok) {
      throw new Error(`Falha no backend legado do Itaú (/rate-limit): HTTP ${rateLimitResponse.status}`)
    }
