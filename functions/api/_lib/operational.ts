...
export async function ensureOperationalTables(db: D1Database) {
  ...
  await db
    .prepare(
      'CREATE INDEX IF NOT EXISTS idx_adminapp_module_events_created_at ON adminapp_module_events(created_at DESC)',
    )
    .run();
  await db
    .prepare(
      'CREATE INDEX IF NOT EXISTS idx_adminapp_module_events_module ON adminapp_module_events(module, created_at DESC)',
    )
    .run();
  ...
  await db
    .prepare(
      'CREATE INDEX IF NOT EXISTS idx_adminapp_sync_runs_module_started ON adminapp_sync_runs(module, started_at DESC)',
    )
    .run();
}
...
export async function startSyncRun(db: D1Database, run: SyncRunStart) {
  ...
  const row = await db
    .prepare('SELECT id FROM adminapp_sync_runs WHERE module = ? ORDER BY id DESC LIMIT 1')
    .bind(run.module)
    .first<{ id?: number }>();
