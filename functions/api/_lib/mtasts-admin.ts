...
const parseJsonOrThrow = <T>(rawText: string, fallback: string, response: Response): T => {
  ...
  if (looksLikeHtml) {
    throw new Error(
      `${fallback}: resposta HTML inesperada do legado (HTTP ${response.status}). Verifique Cloudflare Access/challenge.`,
    );
  }
...
export const postLegacyJson = async <T>(
  env: Env,
  path: string,
  fallback: string,
  body: unknown,
...
export const generateMtastsId = () => {
  ...
  const suffix = Array.from(random)
    .map((chunk) => String(chunk).padStart(10, '0').slice(-4))
    .join('');
